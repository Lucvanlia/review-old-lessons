...
try:
    ...
    java_subj = [s for s in subjects if s["id"] == subject_id]
    if not java_subj:
        errors.append("Java subject is not registered in subjects.json")
    else:
        print("Java subject registration found.")
...
if len(loaded_data) == 3:
    ...
    # Check that each topic has questions
    topic_counts = {}
    for q in questions:
        t_id = q.get("topicId")
        if not t_id:
    ...
    for idx, exam in enumerate(preset_exams):
        ...
        for q in exam_qs:
            q_id = q.get("id")
            # Verify that question matches what is in questions list
            master_q = next((x for x in questions if x["id"] == q_id), None)
            if not master_q:
                errors.append(f"Preset exam '{exam_id}' references question ID {q_id} which does not exist in master questions list.")
...
if errors:
    ...
    print("\nAll Java data files validated successfully! Integrity checks PASSED.")
